...
def bike_return_zone():
    ...
    try:
        ...
        try:
            db.execute(
                ...
                INSERT INTO lock_status
                    (bike_id, lock_state, transferable, is_active, latitude, longitude, created_at)
                VALUES (?, 'locked', 1, 1, ?, ?, ?)
                """,
                (bike_id, lat, lon, end_at)
            )
        except sqlite3.Error:
            pass
        ...
        try:
            db.execute(
                """
                INSERT INTO bike_location_log
                    (bike_id, hub_id, zone_id, latitude, longitude, event_type, created_at)
                VALUES (?, ?, ?, ?, ?, 'return_zone', ?)
                """,
                (bike_id, hub_id, zone["zone_id"], lat, lon, end_at)
            )
        except sqlite3.Error:
            pass
